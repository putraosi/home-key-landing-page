import { Key, Home, Car, Building, Shield, Wrench } from 'lucide-react';
...
const ServicesSection = () => {
  ...
  const handleServiceClick = (serviceTitle: string) => {
    ...
    const message = encodeURIComponent(`Halo, saya tertarik dengan layanan ${serviceTitle}. Bisa minta info lebih lanjut?`);
    window.open(`https://wa.me/62${phone}?text=${message}`, '_blank');
  };

  const handleConsultationClick = () => {
    const phone = PHONE.slice(1).replace(/\-/g, "");
    const message = encodeURIComponent('Halo, saya ingin konsultasi gratis tentang kebutuhan kunci saya');
    window.open(`https://wa.me/62${phone}?text=${message}`, '_blank');
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16 animate-fade-in-up">
          <h2 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">
            Layanan 
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-red-500"> Kami</span>
          </h2>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Berbagai layanan duplikat kunci profesional dengan kualitas terbaik dan harga terjangkau
          </p>
        </div>
